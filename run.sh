...
if [ $# -ne 8 ]; then
	...
	echo "example:"
	echo ""
	echo "   $0 1.0 1.0e+30 14400 mandel_dd 1920 1080 -0.743643887037158704752191506114774 0.131825904205311970493132056385139"
	exit
fi
...
magn=`python2 -c "from math import pow; fac = pow($end / $start, 1.0 / ($frames - 1)); print \"\\n\".join([ str($start * pow(fac, i-1)) for i in range(1, $frames + 1)])"`
